...
import (
	...
	"github.com/priyxstudio/propel/environment/process"
...
func (m *Manager) InitServer(data remote.ServerConfigurationResponse) (*Server, error) {
	...
	settings := environment.Settings{
		...
		Limits:      s.cfg.Build,
		Labels:      s.cfg.Labels,
        StopType:    s.ProcessConfiguration().Stop.Type,
        StopValue:   s.ProcessConfiguration().Stop.Value,
	}
	...
	if runtime.GOOS == "windows" {
		meta := process.Metadata{
			DataDir: s.Filesystem().Path(),
		}
		if env, err := process.New(s.ID(), &meta, envCfg); err != nil {
			return nil, err
		} else {
			s.Environment = env
