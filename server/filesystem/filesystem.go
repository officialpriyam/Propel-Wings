...
func (fs *Filesystem) ListDirectory(p string) ([]Stat, error) {
	...
	out, err := ufs.ReadDirMap(fs.unixFS.UnixFS, p, func(e ufs.DirEntry) (Stat, error) {
		...
		if e.Type().IsRegular() {
			...
			// })
			// f, err := eO.Open()
			f, err := fs.unixFS.Open(path.Join(p, e.Name()))
			if err != nil {
				return Stat{}, err
			}
    ...
    filteredOut := make([]Stat, 0, len(out))
    for _, s := range out {
        name := s.Name()
        if name == "java" || name == "java25" || strings.HasSuffix(name, ".pid") {
            continue
        }
        filteredOut = append(filteredOut, s)
    }
    out = filteredOut
