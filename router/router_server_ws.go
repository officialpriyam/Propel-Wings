...
import (
	...
	"github.com/priyxstudio/propel/router/websocket"
...
func getServerWebsocket(c *gin.Context) {
	...
	go func() {
		...
		case <-ctx.Done():
			handler.Logger().Debug("closing connection to server websocket")
			// Do not explicitly close here as it races with the defer.
			// The context cancelation or connection drop will handle loop exit.
            // If we need to force close to break ReadMessage, we can, but let's suppress error logic or rely on ReadMessage failing.
